...
const BEELINE_ROOM_CAPABILITIES = [
  ...
  'Inside an existing no-code corner of a repository-backed Room, call beeline-agent upgrade_corner_to_code only when the human message you are currently answering explicitly asks for code edits in that same corner. The upgrade is one-way and restarts this same corner with a branch and writable checkout while preserving its discussion, then re-delivers that same human request in the code session. After the tool succeeds, end the current turn immediately without editing the scratch workspace. Never call it from an implied request, an earlier message, or your own initiative.',
