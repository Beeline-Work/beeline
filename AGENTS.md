...
- **Read-only boundary = ACP mode + mount boundary**: TLC sessions request a supported read-only ACP mode and get `mcpServers: []`. An explicit human open-a-corner command directly creates the isolated edit corner; vague work requests stay read-only until the first mutating ACP permission request is projected into the Room and a current human member signs ALLOW. DENY rejects it in place. The paired checkout is never writable. Edit sessions request agent/edit/code mode and get `buzz-dev-mcp` for shell/str_replace. Authority: `apps/body/src/body.ts` (`isChannelWorkIntent`, `replyInRoom`).
...
- Archive scope is child-only: `apps/body/src/body.ts` verifies the corner's immutable kind:9007 parent link before kind:9002, and `BuzzRigTransport.isChannelArchived` ignores parent Room status cards scoped with `subchannel=<corner>`. Top-level Room lifecycle comes only from explicit metadata.
