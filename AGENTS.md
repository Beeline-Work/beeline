...
- Activity projection: body bridges ordered ACP `session/update` batches → kind:9 `#t=agent-activity` channel events for multi-user visibility without exhausting per-key relay quotas. Read-only Room turns also publish `#t=agent-turn` working/complete lifecycle events for the mobile thinking indicator. Room daemon liveness uses one parameterized-replaceable `#t=agent-presence` record per agent/Room with a bounded freshness lease, so heartbeats never crowd chat history. `apps/body/src/activity.ts` owns these wires and the narrow Codex startup-warning filter.
