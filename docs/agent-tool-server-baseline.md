...
Recorded 2026-08-28 and re-run from `fm/update-boundary-nonevent` against current
`origin/main` using the production bubblewrap mount shape.
...
| --------------------------------------------------------------------------------- | ------------- | ------------- | ------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Tool mount + `read_mandate`                                                       | PASS (native) | PASS (native) | PASS (native) | PASS (native) | `probe:agent-tools <harness> --sandbox`; Grok ran through native `grok agent stdio`                                           |
| `open_corner` and simulated retry return one corner id                            | PASS (native) | PASS (native) | PASS (native) | PASS (native) | `probe:agent-tools <harness> --sandbox --workflow`; each transcript contains two calls and the same corner id                 |
| Inline `deliver({name, content, audience})` carries content and returns hash/size | PASS (native) | PASS (native) | PASS (native) | PASS (native) | Sandboxed native workflow transcript; `candidateBytes`/upload integration covers the shared capture boundary                  |
| `close_corner(..., land)` returns the approval-pending business result            | PASS (native) | PASS (native) | PASS (native) | PASS (native) | Sandboxed native workflow transcript returns the same structured `approval_pending` shape                                     |
...
All four harnesses produced the same ordered call sequence under the production
sandbox shape:
...
The visible native names were `mcp.beeline-agent-tools.*` (Codex),
`mcp__beeline-agent-tools__*` (Claude), `beeline-agent-tools_*` (Pi), and
`beeline-agent-tools__*` through Grok's `use_tool` surface. Pi ran through the
generated extension and exact installed release-bundled adapter; no runtime
install occurred.
