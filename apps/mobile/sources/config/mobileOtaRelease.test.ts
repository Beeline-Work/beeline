...
case "$1" in
  ...
  update:list) printf '{"name":"production","id":"branch-id","currentPage":[{"group":"known-good-android-23","platforms":"android","runtimeVersion":"23"},{"group":"known-good-android","platforms":"android","runtimeVersion":"24"},{"group":"known-good-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"known-good-ios","platforms":"ios","runtimeVersion":"25"}]}\\n' ;;
...
`,
    ...
    expect(ledger).toMatchObject({
      ...
      candidateGroupId:
        'candidate-android-23,candidate-android-24,candidate-ios-23,candidate-ios-25',
      androidUpdateId: 'beta-android-23',
      previousProductionGroupId:
        'known-good-android-23,known-good-android,known-good-ios-23,known-good-ios',
      canary: { status: 'passed' },
      production: {
        sourceGroupId:
          'candidate-android-23,candidate-android-24,candidate-ios-23,candidate-ios-25',
        groupId:
          'production-android-23,production-android-24,production-ios-23,production-ios-25',
        targets: [
...
case "$1" in
  channel:view|channel:edit) printf '{}\\n' ;;
  update:list) printf '{"name":"production","id":"branch-id","currentPage":[{"group":"known-good-ios-25","platforms":"ios","runtimeVersion":"25"},{"group":"known-good-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"known-good-android","platforms":"android","runtimeVersion":"24"},{"group":"known-good-android-23","platforms":"android","runtimeVersion":"23"}]}\\n' ;;
  update) printf '[{"id":"beta-%s-%s","platform":"%s","group":"candidate-%s-%s","runtimeVersion":"%s"}]\\n' "$7" "$EXPO_RUNTIME_OVERRIDE" "$7" "$7" "$EXPO_RUNTIME_OVERRIDE" "$EXPO_RUNTIME_OVERRIDE" ;;
  update:republish)
    case "$3" in
      candidate-android-23) printf '[{"id":"prod-next-android-23","platform":"android","group":"production-android-23","runtimeVersion":"23"}]\\n' ;;
...
`,
    ...
    expect(JSON.parse(readFileSync(ledgerPath, 'utf8'))).toMatchObject({
      ...
      candidateGroupId:
        'candidate-android-23,candidate-android-24,candidate-ios-23,candidate-ios-25',
      previousProductionGroupIds: { android: 'known-good-android', ios: 'known-good-ios-25' },
      previousProductionGroupId:
        'known-good-android-23,known-good-android,known-good-ios-23,known-good-ios-25',
      runtimeVersions: ['23', '24', '25'],
    });
    ...
    // All four beta groups are promoted, including both android and both iOS runtimes.
    ...
    expect(JSON.parse(readFileSync(ledgerPath, 'utf8'))).toMatchObject({
      ...
      production: {
        ...
        groupId: 'production-android-23,production-android,production-ios-23,production-ios-25',
      },
    });
    ...
    expect(proof.stdout).toContain(
      'production_group_id=production-android-23,production-android,production-ios-23,production-ios-25',
    );
    expect(proof.stdout).toContain(
      'production_groups=android=production-android,ios=production-ios-25',
    );
    expect(proof.stdout).toContain('production_targets=android@23,android@24,ios@23,ios@25');

    // The delivery target names every group, and the Android device receipt
    ...
    expect(target.stdout.split('\n')[0]).toBe(
      'group_id=production-android-23,production-android,production-ios-23,production-ios-25',
    );
    expect(
      runRelease([
        'confirm',
        ...
        receiptPath,
        '--group',
        'production-android-23,production-android,production-ios-23,production-ios-25',
        '--update-ids',
        'prod-next-android-23,prod-next-android,prod-next-ios-23,prod-next-ios-25',
      ]).status,
    ).toBe(0);
  ...
  it('proves every production target and group through eas update:list', () => {
    ...
    writeFileSync(
      ...
      `#!/bin/sh
case "$1" in
  update:list) printf '{"name":"production","id":"branch-id","currentPage":[{"group":"production-ios-25","platforms":"ios","runtimeVersion":"25"},{"group":"production-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"production-android","platforms":"android","runtimeVersion":"24"}]}\\n' ;;
  *) exit 9 ;;
esac
`,
    );
    ...
    expect(result.stdout).toContain('listed_production_targets=android@24,ios@23,ios@25');
...
`,
    ...
    // binary's embedded update as the anchor for its CURRENT pin only; android@23
    // and ios@23 (the compatibility runtimes) are not the current pins, so both
    // still refuse.
...
case "$1" in
  channel:view|channel:edit) printf '{}\\n' ;;
  update:list) printf '{"currentPage":[{"group":"known-good-android-23","platforms":"android","runtimeVersion":"23"},{"group":"known-good-android","platforms":"android","runtimeVersion":"24"},{"group":"known-good-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"known-good-ios","platforms":"ios","runtimeVersion":"25"}]}\\n' ;;
...
case "$1" in
  ...
  update:list) printf '{"currentPage":[{"group":"known-good-android-23","platforms":"android","runtimeVersion":"23"},{"group":"known-good-android","platforms":"android","runtimeVersion":"24"},{"group":"known-good-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"known-good-ios","platforms":"ios","runtimeVersion":"25"}]}\\n' ;;
  update)
    if [ "$7" = ios ] && [ "$EXPO_RUNTIME_OVERRIDE" = 23 ]; then
      printf '[{"id":"beta-ios","platform":"ios","runtimeVersion":"23"}]\\n'
    else
      printf '[{"id":"beta-%s-%s","platform":"%s","group":"candidate-%s-%s","runtimeVersion":"%s"}]\\n' "$7" "$EXPO_RUNTIME_OVERRIDE" "$7" "$7" "$EXPO_RUNTIME_OVERRIDE" "$EXPO_RUNTIME_OVERRIDE"
    fi ;;
  *) exit 9 ;;
esac
`,
    ...
    expect(
      runRelease(
        [
          ...
          'production-android-23,production-android,production-ios-23,production-ios-25',
          '--update-ids',
          'prod-android-23,prod-android,prod-ios-23,prod-ios-25',
        ],
        env,
      ).status,
    ).toBe(0);
