...
describe('the corner line and the turn indicator are independent', () => {
  ...
  it('renders no stop control and marks the composer as running', () => {
    expect(chatSource).not.toContain('handleStopTurn');
    expect(chatSource).not.toContain("monolithPhoneOperation('cancelAgentTurn'");
    expect(chatSource).toContain('running={Boolean(activeAgentTurn)}');
  });
});
