...
/** Seat displays only — Xvfb typically binds a high :N and is not a screen. */
const SEAT_X_SOCKETS = new Set(['X0', 'X1']);
...
/**
 * The ordinary screen on this machine, if it has one. Virtual displays are
 * not a screen: only the seat sockets (:0 / :1) or an already-set Wayland /
 * seat DISPLAY count.
 */
export function hostSeatDisplay(options?: {
  readonly env?: NodeJS.ProcessEnv;
  readonly x11Dir?: string;
}): string | undefined {
  const env = options?.env ?? process.env;
  const wayland = env.WAYLAND_DISPLAY?.trim();
  const display = env.DISPLAY?.trim();
  if (wayland) return display && isSeatDisplay(display) ? display : ':0';
  if (display && isSeatDisplay(display)) return display;
  const dir = options?.x11Dir ?? '/tmp/.X11-unix';
  try {
    const sockets = readdirSync(dir);
    if (sockets.includes('X0')) return ':0';
    if (sockets.includes('X1')) return ':1';
  } catch {
    return undefined;
  }
  return undefined;
...
export function isSeatDisplay(display: string): boolean {
  const name = display.replace(/^:/, '').split('.')[0];
  return SEAT_X_SOCKETS.has(`X${name}`);
}
