...
describe('MonoMarkdown lists', () => {
  it('keeps document heading levels distinct on the visible inline text', () => {
    ...
    const styleOf = (word: string) => {
      const span = renderer.root.find(
        (node) =>
          node.type === 'Text' && node.children.includes(word) && Array.isArray(node.props.style),
      );
  ...
  it('renders document quotes and linked table cells as selectable, pressable text', () => {
    ...
    expect(
      renderer.root.findAllByProps({ accessibilityRole: 'link' }).length,
    ).toBeGreaterThanOrEqual(2);
    expect(
      renderer.root.findAll((node) => node.props.selectable === true).length,
