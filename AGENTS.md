...
- **Read-only boundary = ACP mode + mount boundary**: TLC sessions request a supported read-only ACP mode, reject native-agent permission escalation, and get `mcpServers: []`; edit sessions request the agent/edit/code mode and get `buzz-dev-mcp` for shell/str_replace. Enforced at ACP `session/new`, not via prompt.
...
- With no `--agent`, `beeline pair` reuses preset resolution to auto-detect ACP-capable `codex`, `claude`, `goose`, and `pi` installations, auto-selects one, or prompts for several on a TTY; non-interactive ambiguity fails closed. `reference` is an explicit-only fallback requiring an LLM key, and `custom` remains explicit-only and shell-free. Preset resolution and install hints live in `apps/body/src/agent-command.ts`; runtime records persist the exact command/argv while retaining legacy `agentBinary` compatibility.
...
- A paired Workspace-member agent creates its deterministic repo Room, makes the pairing human and a dedicated machine-held merge worker admins, then immediately projects itself as member (never admin). The durable daemon's gate discovers every agent-opened change in that Room and auto-lands only exact-tip human-admin approvals. Later Workspace agents join as members and reuse that Room gate. Startup invokes the fail-closed protected-branch assertion after agent membership is established. Live proof: `apps/body/src/pair-runtime.live.test.ts` and `startup-protection.live.test.ts`.
...
- Internal compatibility path: `beeline serve <channel> <owner> <repo>` runs the older explicitly-wired single-Room loop. The supported paired daemon treats every human Room message as a request when the agent is the sole other visible participant; in multi-party Rooms it requires a `p=<agent pubkey>` mention. It opens agent-signed subchannels and never pushes the protected branch. The legacy `t=buzz-agent-request` marker remains wire-compatible but grants no authority by itself.
...
- **Cosmetic identity**: deterministic single-word Agent names/handles live in `packages/buzz-client/src/display-name.ts`; authority-free person profiles and media upload live in `person-profile.ts` and `media.ts`. Mobile defaults are `AgentAvatar.tsx` (chamfered sensor) and `PersonAvatar.tsx` (round orbital mark), with custom images falling back to those marks. Avatar uploads normalize to PNG because the relay rejects metadata-bearing JPEG containers.
...
- Workspace membership is asserted as direct membership in every contained Room for people: invite redemption repairs existing Rooms, and new community-linked Rooms mirror current human members. Registered agents are linked once at Workspace scope but require an explicit per-Room attachment.
