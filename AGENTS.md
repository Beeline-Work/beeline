...
- `beeline pair --agent` selects `codex`, `claude`, `goose`, `pi`, `reference` (default), or a shell-free `custom` command. Preset resolution and install hints live in `apps/body/src/agent-command.ts`; runtime records persist the exact command/argv while retaining legacy `agentBinary` compatibility.
