...
- Supported runtime path: from a source checkout, run `npm run install:beeline` once, then run `beeline pair <code>` inside the target git repo. The installer globally links the built Node CLI and resolves `buzz-agent`/`buzz-dev-mcp` by reusing installed binaries or building the pinned upstream source. `apps/body/src/runtime.ts` normalizes `origin`, resolves/creates that repo's single Workspace Room, persists machine-only state under the git common dir, and launches the durable Room daemon; `beeline start` relaunches it. No `origin` creates a deliberately non-convergent local-only Room.
...
- Internal compatibility path: `beeline serve <channel> <owner> <repo>` runs the older explicitly-wired single-Room loop. The supported paired daemon watches only human `t=buzz-agent-request` messages addressed to its agent key, opens agent-signed subchannels, and never pushes the protected branch.
...
- **Workspace/Room navigation** (`apps/mobile/sources/app/(app)/buzz/channels.tsx`): the person-facing unit for an agent subchannel is a **corner**. Rooms nest a short Slack-style corner preview and link to `buzz/corners/[roomId].tsx` for the complete status list; corner control events stay out of the Room transcript. Wire names remain `subchannel`/`change-review` for compatibility.
