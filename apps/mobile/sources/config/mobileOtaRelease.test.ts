...
function runRelease(args: string[], env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [releaseScript, ...args], {
    ...
    env: {
      ...process.env,
      // This branch intentionally introduces new native runtimes and rides a
      // store build, whose embedded updates are their first rollback anchors.
      OTA_EMBEDDED_ANCHOR_PLATFORMS: 'android,ios',
      ...env,
    },
  });
}
...
describe('mobile OTA release governor', () => {
  it('publishes the committed compatibility runtime targets alongside the store pins', () => {
    ...
    expect(releaseUpdateTargets(mobileRoot)).toEqual([
      { platform: 'android', runtimeVersion: '27' },
      { platform: 'android', runtimeVersion: '23' },
      { platform: 'android', runtimeVersion: '24' },
      { platform: 'android', runtimeVersion: '25' },
      { platform: 'android', runtimeVersion: '26' },
      ...
      { platform: 'ios', runtimeVersion: '26' },
      { platform: 'ios', runtimeVersion: '27' },
      { platform: 'ios', runtimeVersion: '28' },
    ]);
  });

  it('covers every shipped or submitted native runtime, including runtimes the pins moved away from', () => {
    // Regression for #1353/#1383: the pins moved android 25→24 and ios 26→25,
    // and the compat list silently shrank with them, stranding every install
    // on the android@25 / ios@26 store binaries. The union (pins ∪ compat)
    // must keep those runtimes covered, and the pre-publication governor must
    // refuse any plan that does not cover every shipped or submitted runtime.
    const currentPins = { android: '27', ios: '28' };
  ...
  it('fails publish planning before any EAS call when the plan strands a shipped runtime', () => {
    const directory = mkdtempSync(join(tmpdir(), 'beeline-ota-coverage-'));
    // A patched copy of the release script whose compat list dropped
    // android@25 must fail before the first EAS command is even printed.
    ...
    for (const helper of ['ota-delivery-index.mjs', 'native-fingerprint.mjs']) {
      writeFileSync(
        ...
        readFileSync(join(mobileRoot, 'scripts', helper), 'utf8'),
      );
    }
    const result = spawnSync(
      ...
      {
        // The patched script reads the project config from the repo; the
        // governor reads its runtime lists from the patched file itself.
        cwd: mobileRoot,
        encoding: 'utf8',
      },
    );
  ...
  it('dry-runs beta publish and exact-group production operations with pinned EAS CLI', () => {
    ...
    expect(targetPublishes).toHaveLength(11);
    expect(targetPublishes[0]).toContain('EXPO_RUNTIME_OVERRIDE=27');
    ...
    expect(targetPublishes[5]).toContain('EXPO_RUNTIME_OVERRIDE=23');
    expect(targetPublishes[5]).toContain('--platform ios');
    expect(targetPublishes[6]).toContain('EXPO_RUNTIME_OVERRIDE=24');
    expect(targetPublishes[6]).toContain('--platform ios');
    expect(targetPublishes[7]).toContain('EXPO_RUNTIME_OVERRIDE=25');
    expect(targetPublishes[7]).toContain('--platform ios');
    expect(targetPublishes[8]).toContain('EXPO_RUNTIME_OVERRIDE=26');
    expect(targetPublishes[8]).toContain('--platform ios');
    expect(targetPublishes[9]).toContain('EXPO_RUNTIME_OVERRIDE=27');
    expect(targetPublishes[9]).toContain('--platform ios');
    expect(targetPublishes[10]).toContain('EXPO_RUNTIME_OVERRIDE=28');
    expect(targetPublishes[10]).toContain('--platform ios');
    expect(publish.stdout).toContain(
      'release_plan_targets=android@27,android@23,android@24,android@25,android@26,ios@23,ios@24,ios@25,ios@26,ios@27,ios@28',
    );
...
case "$1" in
  ...
  update:republish)
    case "$3" in
      candidate-android-27) printf '[{"id":"prod-next-android-27","platform":"android","group":"production-android-27","runtimeVersion":"27"}]\\n' ;;
      ...
      candidate-ios-28) printf '[{"id":"prod-next-ios-28","platform":"ios","group":"production-ios-28","runtimeVersion":"28"}]\\n' ;;
      *) exit 9 ;;
    esac ;;
  *) exit 9 ;;
esac
`,
    ...
    expect(ledger).toMatchObject({
      ...
      candidateGroupId:
        'candidate-android-27,candidate-android-23,candidate-android-24,candidate-android-25,candidate-android-26,candidate-ios-23,candidate-ios-24,candidate-ios-25,candidate-ios-26,candidate-ios-27,candidate-ios-28',
      androidUpdateId: 'beta-android-27',
      ...
      production: {
        sourceGroupId:
          'candidate-android-27,candidate-android-23,candidate-android-24,candidate-android-25,candidate-android-26,candidate-ios-23,candidate-ios-24,candidate-ios-25,candidate-ios-26,candidate-ios-27,candidate-ios-28',
        groupId:
          'production-android-27,production-android-23,production-android-24,production-android-25,production-android-26,production-ios-23,production-ios-24,production-ios-25,production-ios-26,production-ios-27,production-ios-28',
        targets: [
          { platform: 'android', runtimeVersion: '27', group: 'production-android-27' },
          ...
          { platform: 'ios', runtimeVersion: '28', group: 'production-ios-28' },
        ],
      },
    });
  }, 60_000);
...
`,
    ...
    expect(JSON.parse(readFileSync(ledgerPath, 'utf8'))).toMatchObject({
      status: 'beta',
      candidateGroupIds: { android: 'candidate-android-27', ios: 'candidate-ios-28' },
      candidateGroupId:
        'candidate-android-27,candidate-android-23,candidate-android-24,candidate-android-25,candidate-android-26,candidate-ios-23,candidate-ios-24,candidate-ios-25,candidate-ios-26,candidate-ios-27,candidate-ios-28',
      ...
      runtimeVersions: ['27', '23', '24', '25', '26', '28'],
    });
    ...
    // Every current and compatibility target is promoted independently.
    const republished = readFileSync(callsPath, 'utf8')
      .split('\n')
      .filter((line) => line.startsWith('update:republish'));
    expect(republished).toHaveLength(11);
    expect(republished[0]).toContain('--group candidate-android-27');
    ...
    expect(republished[5]).toContain('--group candidate-ios-23');
    expect(republished[6]).toContain('--group candidate-ios-24');
    expect(republished[7]).toContain('--group candidate-ios-25');
    expect(republished[8]).toContain('--group candidate-ios-26');
    expect(republished[9]).toContain('--group candidate-ios-27');
    expect(republished[10]).toContain('--group candidate-ios-28');

    expect(JSON.parse(readFileSync(ledgerPath, 'utf8'))).toMatchObject({
      status: 'production',
      production: {
        sourceGroupIds: { android: 'candidate-android-27', ios: 'candidate-ios-28' },
        groupIds: { android: 'production-android-27', ios: 'production-ios-28' },
        groupId:
          'production-android-27,production-android-23,production-android,production-android-25,production-android-26,production-ios-23,production-ios-24,production-ios-25,production-ios-26,production-ios-27,production-ios-28',
      },
    });
    expect(JSON.parse(readFileSync(indexPath, 'utf8')).merges.at(-1).published).toMatchObject({
      groupIds: [
        'production-android-27',
    ...
    expect(proof.stdout).toContain(
      'production_group_id=production-android-27,production-android-23,production-android,production-android-25,production-android-26,production-ios-23,production-ios-24,production-ios-25,production-ios-26,production-ios-27,production-ios-28',
    );
    expect(proof.stdout).toContain(
      'release_targets=android@27,android@23,android@24,android@25,android@26,ios@23,ios@24,ios@25,ios@26,ios@27,ios@28',
    );
    expect(proof.stdout).toContain(
      'production_groups=android=production-android-27,ios=production-ios-28',
    );
    expect(proof.stdout).toContain(
      'production_targets=android@27,android@23,android@24,android@25,android@26,ios@23,ios@24,ios@25,ios@26,ios@27,ios@28',
    );
        ...
        devices: [
          {
            ...
            group: 'production-android-27',
            updateId: 'prod-next-android-27',
    ...
    expect(target.stdout.split('\n')[0]).toBe(
      'group_id=production-android-27,production-android-23,production-android,production-android-25,production-android-26,production-ios-23,production-ios-24,production-ios-25,production-ios-26,production-ios-27,production-ios-28',
    );
    expect(
      runRelease([
        'confirm',
        ...
        'production-android-27,production-android-23,production-android,production-android-25,production-android-26,production-ios-23,production-ios-24,production-ios-25,production-ios-26,production-ios-27,production-ios-28',
        '--update-ids',
        'prod-next-android-27,prod-next-android-23,prod-next-android,prod-next-android-25,prod-next-android-26,prod-next-ios-23,prod-next-ios-24,prod-next-ios-25,prod-next-ios-26,prod-next-ios-27,prod-next-ios-28',
      ]).status,
    ).toBe(0);
    expect(JSON.parse(readFileSync(indexPath, 'utf8')).merges.at(-1)).toMatchObject({
      state: 'confirmed',
      confirmed: {
        groupId: 'production-android-27',
        groupIds: [
          'production-android-27',
          'production-android-23',
          'production-android',
  ...
  it('proves every production target and group through eas update:list', () => {
    ...
    writeFileSync(
      ...
      `#!/bin/sh
case "$1" in
  update:list) printf '{"name":"production","id":"branch-id","currentPage":[{"group":"production-ios-25","platforms":"ios","runtimeVersion":"25"},{"group":"production-ios-23","platforms":"ios","runtimeVersion":"23"},{"group":"production-android","platforms":"android","runtimeVersion":"24"},{"group":"production-android-23","platforms":"android","runtimeVersion":"23"}]}\\n' ;;
  *) exit 9 ;;
esac
`,
    );
    ...
    expect(result.stdout).toContain(
      'listed_production_targets=android@24,android@23,ios@23,ios@25',
    );
...
`,
    ...
    for (const key of ['android@23', 'android@25', 'android@26', 'ios@23', 'ios@24', 'ios@25', 'ios@26', 'ios@27']) {
    ...
    expect(incomplete.stdout).toContain(
      'android@27: first production release on this runtime; rollback anchor = embedded update of this release\'s store binary',
    );
    expect(incomplete.stdout).toContain(
      'ios@28: first production release on this runtime; rollback anchor = embedded update of this release\'s store binary',
    );
...
`,
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('Beta publish android@27 must return exactly android@27');
...
`,
    ...
    expect(result.stderr).toContain(
      'Beta publish android@27 returned updates that belong to no update group: ios/beta-ios.',
    );
  }, 60_000);
...
case "$1" in
  ...
  update:republish)
    case "$3" in
      candidate-android-27) printf '[{"id":"prod-android-27","platform":"android","group":"production-android-27","runtimeVersion":"27"}]\\n' ;;
      ...
      candidate-ios-28) printf '[{"id":"prod-ios-28","platform":"ios","group":"production-ios-28","runtimeVersion":"28"}]\\n' ;;
      *) exit 9 ;;
    esac ;;
  *) exit 9 ;;
esac
