...
test('the unified release publishes every stable website installer name', () => {
  ...
  assert.match(releaseWorkflow, /needs: \[initialize, server, helper, mobile_ota, mobile_native_android, mobile_native_ios, desktop_installers/);
  for (const { asset } of DESKTOP_INSTALLERS) {
    assert.match(desktopWorkflow, new RegExp(asset.replace('.', '\\.')));
  }
  assert.match(releaseWorkflow, /mapfile -t assets < <\(find "\$RUNNER_TEMP\/desktop-release"/);
  assert.match(releaseWorkflow, /gh release create[\s\S]*"\$\{release_assets\[@\]\}"/);
