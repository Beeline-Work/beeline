...
describe('monolith integration', () => {
  ...
  it('rejects stale agent continuity when the trigger agent is answering a human', async () => {
    const hoots = 'e'.repeat(64);
    await database.query(
      `INSERT INTO identities(id,kind,name,handle) VALUES($1,'agent','Hoots','hoots')`,
      [hoots],
    );
    await database.query(`INSERT INTO agents(agent_id,owner_id) VALUES($1,$2)`, [hoots, HUMAN]);
    await database.query(
      `INSERT INTO memberships(workspace_id,room_id,identity_id,role)
       VALUES($1,NULL,$2,'member'),($1,$3,$2,'member')`,
      [WORKSPACE, hoots, ROOM],
    );
    const hootsExchange = await auth.createDaemonExchange(hoots);
    const hootsToken = (await auth.exchangeDaemonToken(hootsExchange.exchangeToken))!.daemonToken;

    const olderHumanRequest = '6'.repeat(64);
    await operation('sendRoomMessage', {
      roomId: ROOM,
      messageId: olderHumanRequest,
      text: '@hoots explain who sees this.',
      mentions: [hoots],
    });
    const hootsMentionsGoosy = await daemonOperation(
      'postRoomMessage',
      {
        roomId: ROOM,
        requestId: olderHumanRequest,
        triggerMessageId: olderHumanRequest,
        text: 'Tag @bee and Bee answers.',
        mentionIds: [AGENT],
      },
      hootsToken,
    );
    expect(hootsMentionsGoosy.status).toBe(200);
    const hootsMentionId = ((await hootsMentionsGoosy.json()) as { id: string }).id;
    expect(
      (
        await daemonOperation('postRoomMessage', {
          roomId: ROOM,
          requestId: hootsMentionId,
          triggerMessageId: hootsMentionId,
          text: 'Older Goosy answer.',
        })
      ).status,
    ).toBe(200);

    const currentHumanRequest = '7'.repeat(64);
    await operation('sendRoomMessage', {
      roomId: ROOM,
      messageId: currentHumanRequest,
      text: '@hoots answer this new question.',
      mentions: [hoots],
    });
    const hootsAnswer = await daemonOperation(
      'postRoomMessage',
      {
        roomId: ROOM,
        requestId: currentHumanRequest,
        triggerMessageId: currentHumanRequest,
        text: 'Hoots answers the human without tagging another agent.',
      },
      hootsToken,
    );
    expect(hootsAnswer.status).toBe(200);
    const hootsAnswerId = ((await hootsAnswer.json()) as { id: string }).id;

    const staleSecondHop = await daemonOperation('postRoomMessage', {
      roomId: ROOM,
      requestId: hootsAnswerId,
      triggerMessageId: hootsAnswerId,
      text: 'Goosy must not publish this stale second-hop answer.',
    });
    expect(staleSecondHop.status).toBe(400);
    await expect(staleSecondHop.json()).resolves.toEqual({
  ...
  it('keeps explicit agent mentions on replies to humans', async () => {
    ...
    const sent = await operation('sendRoomReply', {
      ...
      messageId: replyId,
      parentMessageId: parentId,
      text: '@bee This one is explicitly for the agent.',
      mentions: [AGENT],
    });
    expect(sent.status).toBe(200);
    const stored = await database.query<{ mention_ids: string[] }>(
