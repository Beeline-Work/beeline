...
describe('native fingerprint gate', () => {
  it('pins the runtime version by hand, because a computed stamp orphans installed binaries', () => {
    ...
    expect(readPinnedRuntimeVersion(mobileRoot)).toEqual({ android: '28', ios: '29' });
    expect(runtimeVersionsFromConfig(baseline)).toEqual({ android: '28', ios: '29' });
  });
  ...
  it(
    ...
    async () => {
      ...
      expect(runtimeVersionsFromConfig(before)).toEqual({ android: '28', ios: '29' });
...
module.exports = withoutIosPushCapabilities;\n`,
      ...
      expect(dirty.stderr).toContain('Native inputs changed but android.runtimeVersion is still "28"');
      ...
      expect(dirty.stderr).toContain('bump android.runtimeVersion in apps/mobile/app.config.js to "29"');
      ...
      expect(refused.stderr).toContain('Native inputs changed but android.runtimeVersion is still "28"');
