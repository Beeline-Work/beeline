...
describe('GitHub-only mobile sign-in surface', () => {
  it('starts monolith sign-in on the monolith auth host', () => {
    expect(source('app/(app)/beeline/onboarding.tsx')).toContain(
      'startGitHubSignInWebFlow(state, runtime)',
    );
  });

  it('has no legacy callback routes or provider-selection module', () => {
    expect(existsSync(new URL('../app/(app)/beeline/oidc-callback.tsx', import.meta.url))).toBe(
      false,
    );
    expect(existsSync(new URL('../app/(app)/auth/oidc/mobile-callback.tsx', import.meta.url))).toBe(
      false,
    );
  ...
  it('leaves no legacy-named sign-in symbol or provider branch in mobile sources', () => {
    const signInSources = [
      ...
      source('app/(app)/beeline/chat/chat-surface.tsx'),
