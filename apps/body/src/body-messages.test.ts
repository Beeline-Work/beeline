...
describe('first-class assistant messages', () => {
  ...
  it('reserves target synchronization for one explicit model-owned follow-up turn', () => {
    const source = readFileSync(new URL('./body.ts', import.meta.url), 'utf8');
    ...
    expect(source).toMatch(/moved to \$\{targetTip\}[^`]*make it merge-ready, whatever it takes/i);
    ...
    // the daemon detects an exact moved target generation.
