...
import { HullFloatingSurface } from './buzz/HullDialog';
...
export const DuplicateSheet = React.memo(function DuplicateSheet(props: DuplicateSheetProps) {
  ...
  const source = React.useMemo(
    () => (session ? getSessionForkSource(session) : null),
    [
      session?.id,
  ...
  const selected = points && selectedId ? (points.find((p) => p.id === selectedId) ?? null) : null;
  ...
  const [loading, doDuplicate] = useHappyAction(async () => {
    ...
    const forkedFromMessageId = matchesInitialSelection(
      selected,
      initialSelectedId,
      initialMessageText,
    )
  ...
  return (
    <HullFloatingSurface style={[styles.sheet, sheetFrame]}>
          ...
          points.map((p) => {
            ...
            return (
              ...
              >
                ...
                <Text style={styles.rowMeta}>{formatRelativeTime(p.timestamp)}</Text>
              </Pressable>
            );
          })
        )}
