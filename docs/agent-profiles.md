...
The owner uses Edit, Save, and Cancel in the profile to change the name and soul. Close and Message confirm before discarding changed text. The edit state previews the assigned animal with a generated appearance seeded by the draft soul; Save persists that seed. The edit state also exposes the existing model/effort, Answers everyone, and Yolo controls; those changes apply immediately. Public Workspace policy still forces Yolo off. Workspace managers may remove/ban an agent but gain no owner-only configuration powers. Ordinary members see a read-only profile. The animal itself remains server assigned; this change does not introduce photo uploads or a new agent-face permission.
