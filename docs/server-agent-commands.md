...
Schedules, event subscriptions, corner objectives/check changes, grant decisions and cancellation produce explicit input/resume/stop actions. A corner's one-way `no_code -> code` lane upgrade produces a resume too, and is the one producer that does not keep the original request id: it re-arms the human's own command row to pending under `lane-upgrade:<request>` with `reason='corner_lane_upgrade'` — its own row when that request was already a resume, since the row key is `(room, message, agent, action)` — so the same ask is delivered again once the corner has restarted with a checkout. Resumptions otherwise retain the original request and root; check-state commands are coalesced by stored lifecycle state. A parent Room's configured reviewer is subscribed to `check-passed` in that Room and all of its corners. Each transition to green wakes that reviewer once; failed checks still wake the corner opener. The reviewer reviews the exact green PR head, records a head-bound approval with `approve_merge`, and tags the implementer to merge. A new head needs a new approval. Without a configured reviewer, green-check routing remains with the opener.
