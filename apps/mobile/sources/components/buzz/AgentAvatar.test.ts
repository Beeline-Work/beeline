...
describe('Agent avatar', () => {
  ...
  /**
   * Regression guard for the enter-room/live-update freeze: AgentAvatar
  ...
  it('does not re-render when its own props are unchanged', () => {
    const original = (AgentAvatar as unknown as { type: typeof AgentAvatar }).type;
    const spy = vi.fn(original);
    (AgentAvatar as unknown as { type: typeof AgentAvatar }).type = spy as any;
    try {
      function Parent({ tick }: { tick: number }) {
        void tick;
        return React.createElement(AgentAvatar, { pubkey, name: 'Joy' });
      }
      ...
      (AgentAvatar as unknown as { type: typeof AgentAvatar }).type = original;
    }
  });
});
