...
async function main(): Promise<void> {
  ...
  let ready = false;
  for (let i = 0; i < 20; i++) {
    const r = await gitAuthed(tmpdir(), worker, owner, repo, ['ls-remote', url]);
    if (r.ok) {
      ready = true;
      break;
    }
  ...
  await commit(seed, 'README.md', '# gate proof\n', 'initial commit');
  const seedPush = await gitAuthed(seed, worker, owner, repo, ['push', url, 'main']);
  assert(seedPush.ok, 'worker seeds main (owner push)', seedPush.ok ? '' : seedPush.stderr.trim());
  const baseMain = await lsRemoteRef(seed, worker, owner, repo, 'refs/heads/main');
  ...
  const cloneRes = await gitAuthed(agentRoot, agent, owner, repo, ['clone', url, 'work']);
  assert(
    cloneRes.ok,
    'agent clones the repo (member read)',
    cloneRes.ok ? '' : cloneRes.stderr.trim(),
  );
  const agentWork = join(agentRoot, 'work');
  const featureBranch = 'feature/agent-change';
  await git(agentWork, ['checkout', '-q', '-b', featureBranch]);
  await commit(agentWork, 'CHANGE.txt', 'the agent made this change\n', 'agent: make the change');
  const featureTip = (await git(agentWork, ['rev-parse', 'HEAD'])).stdout.trim();
  console.log('feature tip =', featureTip);

  // A. Agent pushes the FEATURE branch -> allowed (legit member, branch create).
  const pushFeature = await gitAuthed(agentWork, agent, owner, repo, [
    'push',
    'origin',
    featureBranch,
  ]);
  ...
  const pushMain = await gitAuthed(agentWork, agent, owner, repo, ['push', 'origin', `HEAD:main`]);
  const rejected =
    !pushMain.ok || /rejected|denied|forbidden|push:admin|admin role/i.test(pushMain.stderr);
  assert(
    rejected,
    'B. agent push to main -> REJECTED by relay [CRITICAL]',
    pushMain.stderr.trim().split('\n').slice(-2).join(' | '),
  );
  const mainAfterAgent = await lsRemoteRef(seed, worker, owner, repo, 'refs/heads/main');
  assert(
    mainAfterAgent === baseMain,
    'B. main did NOT advance from the agent push',
    `main=${mainAfterAgent}`,
  );
  ...
  const mergeReq = {
    ...
    trustedReviewerCustody: 'device' as const,
    repo,
    channelId,
    targetBranch: 'main',
    featureBranch,
  ...
  assert(
    (await lsRemoteRef(seed, worker, owner, repo, 'refs/heads/main')) === baseMain,
    'C. main unchanged',
  );
  ...
  const finalMain = await lsRemoteRef(seed, worker, owner, repo, 'refs/heads/main');
  assert(
    finalMain === featureTip,
    'E. main advanced to the approved feature tip',
    `main=${finalMain}`,
  );
