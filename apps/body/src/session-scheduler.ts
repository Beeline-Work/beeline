...
export class SessionScheduler {
  ...
  constructor(options: { maxLiveSessions?: number; idleMs?: number } = {}) {
    this.maxLiveSessions = options.maxLiveSessions ?? 4;
    this.idleMs = options.idleMs ?? 5 * 60_000;
    if (!Number.isSafeInteger(this.maxLiveSessions) || this.maxLiveSessions < 1) {
  ...
  run<T>(key: string, lifecycle: SessionLifecycle, task: () => Promise<T>): Promise<T> {
    ...
    return previous
      .catch(() => undefined)
      .then(async () => {
        await this.ensureLive(key, lifecycle);
  ...
  private async ensureLive(key: string, lifecycle: SessionLifecycle): Promise<void> {
    ...
    try {
      ...
      while (this.live.size >= this.maxLiveSessions) {
