...
describe('native fingerprint gate', () => {
  it('pins the runtime version by hand, because a computed stamp orphans installed binaries', () => {
    ...
    expect(baseline.runtimeVersion).toBe('24');
  });
  ...
  it(
    ...
    async () => {
      ...
      expect(before.runtimeVersion).toBe('24');

      appendFileSync(join(project, 'plugins/withEinkCompatibility.js'), '\n// native change\n');
      const dirty = await runGate(fixture);

      expect(dirty.status).toBe(1);
      expect(dirty.stderr).toContain('Native inputs changed but runtimeVersion is still "24"');
      ...
      expect(dirty.stderr).toContain('bump runtimeVersion in apps/mobile/app.config.js to "25"');
      ...
      expect(refused.stderr).toContain('Native inputs changed but runtimeVersion is still "24"');
