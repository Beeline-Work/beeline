...
rsync -a -O --no-o --no-g --delete "$REPO_WEB/" "$STAGE/web/"
...
rsync -a -O --no-o --no-g "$WEBROOT/" "$BAK/"

log "swapping staged tree into $WEBROOT"
rsync -a -O --no-o --no-g --delete "$STAGE/web/" "$WEBROOT/"
...
diff -r --brief "$STAGE/web" "$WEBROOT" >/dev/null || {
  ...
  rsync -a -O --no-o --no-g --delete "$BAK/" "$WEBROOT/"
