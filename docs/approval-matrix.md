...
Every corner turn checks the live repository gate before running its prompt. The current corner harness also has host shell access, so it checks a separate host grant before starting or resuming the autonomous harness; repository permission never grants machine access. Under yolo, the owner bypasses this host prompt, while another requester needs private owner approval. This deliberately applies even to a repository task running in that host-capable harness. A turn stopped before its harness by an unapproved gate is stated by the server, not written by the agent: `DaemonService.noteBlockedTurnGate` inscribes one corner system line — `<agent> asked @<owner> to approve host access`, or `<agent> asked a Workspace admin to approve repository access` — once per pending grant per turn: always when the grant was already pending, and for a freshly minted host grant, whose card goes to the owner's private `@system` DM (a freshly minted repository grant instead posts its own actionable card in that corner). The helper posts no reply for such a turn, so a blocked gate never reads as the agent having nothing to add. Approval does not replace the existing exact-head reviewer/CI/human-hold merge gate. Top-level Rooms retain their read-only filesystem boundary.

Coverage: server `integration.test.ts` (matrix, private decisions, target scope, delegation/resume, revoked and Once grants, command reuse), `corner-review-dispatch.test.ts` (blocked-gate line), `wallet.integration.test.ts`; body `grant-runner.test.ts`, `host-mcp-route.test.ts`, `monolith-corner-turn.test.ts`, Room/corner permission tests; mobile `RoomMessageVariants.test.tsx`.
