...
export function agentMemoryInstructions(memory: AgentMemory | undefined): string {
  ...
  return [
    ...
    AGENT_MEMORY_CURATION_CONTRACT,
    'This memory is YOURS: persist it with beeline-readonly-mcp.write_memory whenever you learn something else worth keeping across conversations (decisions, preferences, lessons, project facts). Read the current file first, then pass the complete curated contents. Shell writes to memory are always denied.',
    ...
    'Writing through beeline-readonly-mcp.write_memory is always permitted, including in read-only Rooms. It is still not a license to write anywhere else: repository paths stay read-only there, and memory must never hold secrets, credentials, or repository files.',
  ].join('\n');
}
