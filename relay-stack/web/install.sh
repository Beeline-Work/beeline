...
for path in bin/beeline bin/buzz-agent bin/buzz-dev-mcp bin/beeline-readonly-mcp bin/codegraph lib/beeline/beeline-cli.mjs lib/beeline/beeline-readonly-mcp.mjs lib/beeline/pi-mcp-adapter.mjs lib/beeline/codegraph/bin/codegraph lib/beeline/bundle.json; do
...
for tool in buzz-agent buzz-dev-mcp beeline-readonly-mcp codegraph; do
...
: "\${BEELINE_CODEGRAPH_BIN:=\$prefix_dir/lib/beeline/bin/codegraph}"
...
echo "beeline installer: installed beeline, buzz-agent, buzz-dev-mcp, beeline-readonly-mcp, and codegraph in $bin_dir"
case ":${PATH:-}:" in
  *:"$bin_dir":*) ;;
