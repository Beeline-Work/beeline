...
describe('native fingerprint gate', () => {
  it('pins the runtime version by hand, because a computed stamp orphans installed binaries', () => {
    ...
    expect(readPinnedRuntimeVersion(mobileRoot)).toEqual({ android: '24', ios: '25' });
    expect(runtimeVersionsFromConfig(baseline)).toEqual({ android: '24', ios: '25' });
  });
  ...
  it(
    ...
    async () => {
      ...
      expect(runtimeVersionsFromConfig(before)).toEqual({ android: '24', ios: '25' });
...
module.exports = withoutIosPushCapabilities;\n`,
      ...
      expect(legacyStripper.stderr).toContain(
        'Native inputs changed but ios.runtimeVersion is still "25"',
      );
      expect(legacyStripper.stderr).toContain(
      ...
      expect(dirty.stderr).toContain('Native inputs changed but android.runtimeVersion is still "24"');
      ...
      expect(dirty.stderr).toContain('bump android.runtimeVersion in apps/mobile/app.config.js to "25"');
      ...
      expect(refused.stderr).toContain('Native inputs changed but android.runtimeVersion is still "24"');
