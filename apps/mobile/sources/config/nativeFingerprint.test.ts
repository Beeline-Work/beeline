...
describe('native fingerprint gate', () => {
  it('pins the runtime version by hand, because a computed stamp orphans installed binaries', () => {
    ...
    expect(appConfig).toContain('runtimeVersion: runtimeVersionOverride || "25"');
    expect(appConfig).not.toContain('policy: "fingerprint"');
    expect(readPinnedRuntimeVersion(mobileRoot)).toEqual({ android: '25', ios: '26' });
    expect(runtimeVersionsFromConfig(baseline)).toEqual({ android: '25', ios: '26' });
  });
  ...
  it(
    ...
    async () => {
      ...
      expect(runtimeVersionsFromConfig(before)).toEqual({ android: '25', ios: '26' });
...
module.exports = withoutIosPushCapabilities;\n`,
      ...
      expect(legacyStripper.stderr).toContain(
        'Native inputs changed but ios.runtimeVersion is still "26"',
      );
      expect(legacyStripper.stderr).toContain(
      ...
      expect(dirty.stderr).toContain('Native inputs changed but android.runtimeVersion is still "25"');
      ...
      expect(dirty.stderr).toContain('bump android.runtimeVersion in apps/mobile/app.config.js to "26"');
      ...
      expect(refused.stderr).toContain('Native inputs changed but android.runtimeVersion is still "25"');
