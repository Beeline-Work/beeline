...
async function pushFeatureBranch(
  ...
  await git(work, ['checkout', '-q', '-b', branchName]);
  await commit(work, 'CHANGE.txt', `agent change for ${branchName}\n`, `agent: ${branchName}`);
  const featureTip = (await git(work, ['rev-parse', 'HEAD'])).stdout.trim();
  const pushFeature = await gitAuthed(work, agent, owner, repo, ['push', 'origin', branchName]);
...
(reachable ? describe : describe.skip)('live failure-mode suite (tests 1-3)', () => {
  ...
  it('1a. approval signed by AGENT key (not the trusted reviewer) is REFUSED and main unchanged', async () => {
    const p = await provisionFresh('fm1');
    const branch = 'feature/fm1-agent-approval';

    const { featureTip } = await pushFeatureBranch(p.agent, p.owner, p.repo, p.url, branch);
  ...
  it('1b. POSITIVE CONTROL: same approval signed by the HUMAN REVIEWER key DOES merge', async () => {
  ...
  it('2a. no approval published → worker refuses and main unchanged', async () => {
  ...
  it('2b. publish valid approval AFTER the initial refusal → merge succeeds', async () => {
  ...
  it('3a. approval for repo A does NOT authorize merge B (different repo)', async () => {
  ...
  it('3b. approval at tip A lands tip C after ongoing commits in the same corner', async () => {
    ...
    const work = join(agentRoot, 'work');
    await git(work, ['fetch', 'origin', branch]);
    await git(work, ['checkout', branch]);
    await commit(work, 'SECOND.txt', 'second commit\n', 'agent: second commit');
    const secondTip = (await git(work, ['rev-parse', 'HEAD'])).stdout.trim();
    const pushSecond = await gitAuthed(work, p.agent, p.owner, p.repo, ['push', 'origin', branch]);
  ...
  it('3c. a different corner cannot reuse the approval', async () => {
