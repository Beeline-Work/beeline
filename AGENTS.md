...
- **Read-only boundary = ACP mode + mount boundary**: TLC sessions request a supported read-only ACP mode and get `mcpServers: []`; their first mutating ACP permission request is projected into the Room. A current human member's signed ALLOW opens the corner/worktree and replays the request in a new edit session; DENY rejects it in place. The paired checkout is never writable. Edit sessions request agent/edit/code mode and get `buzz-dev-mcp` for shell/str_replace.
...
- A paired Workspace-member agent creates its deterministic repo Room, makes the pairing human admin, then immediately projects itself as member (never admin). Buzz-relay origins also provision the dedicated merge worker, enforce the protected-push assertion, and auto-land only exact-tip human-admin approvals. User-hosted git origins omit that gate and land directly with the operator's ambient git credentials; host branch protection is authoritative. Live relay proof: `apps/body/src/pair-runtime.live.test.ts` and `startup-protection.live.test.ts`.
