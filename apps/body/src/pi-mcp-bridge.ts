 ...
 * object and never reads it again. Re-measured on pi-acp 0.0.33 + pi 0.85.1:
 * `this.mcpServers` is assigned in `PiAcpSession` and never read, so
 * `session/new` is still dead. Pi 0.85.1 itself has no MCP client either —
 * an isolated `$PI_CODING_AGENT_DIR/mcp.json` with a live stdio probe did
 * not appear in `pi.getAllTools()` (only `read`/`bash`/`edit`/`write`/
 * `grep`/`find`/`ls`). Optional `pi-mcp-adapter` (operator `settings.json`
 * `packages`) is what reads that file on an interactive host; isolated
 * Beeline homes exclude `settings.json`, so they never load it. The
 * visible consequence was a
